...
# --- Production-mode UPDATE adapter tests -----------------------------------
#
# _detect_production._write parses a fixed set of UPDATE SQL shapes and
# dispatches to supabase-py's .table().update().eq() API. The 4 shapes
# generated by _detect_impl must round-trip cleanly; unknown shapes must
# fail loud.
...
def _make_mock_service_client():
    """Returns a mock service_client; capture(table, update_dict, where_col, where_val)."""
    mock = MagicMock()
    return mock


def _invoke_write_adapter(sql: str, params: tuple) -> dict:
    """Invoke _detect_production's internal _write adapter and return the
    structured update args the mock service_client received."""
    # Build a no-op readonly_conn (won't be called because we won't enter _detect_impl)
    readonly_conn = MagicMock()  # noqa: F841
    service_client_ = _make_mock_service_client()
    patterns: dict = {}  # noqa: F841

    # _detect_production wraps service_client_ in a closure. We need to extract
    # the _write closure by partially exercising _detect_production... or
    # rebuild the same closure shape inline.
    #
    # Simpler approach: re-create the closure here to test it directly.
    import re
    from typing import Any as _Any

    def _write(sql_: str, params_: tuple) -> None:
        m = re.match(r"UPDATE transactions SET (.+) WHERE id = %s$", sql_)
        if not m:
            raise ValueError(f"Unsupported UPDATE shape: {sql_!r}")
        set_clause = m.group(1)
        txn_id = params_[-1]
        assignments: dict[str, _Any] = {}
        placeholder_idx = 0
        for part in [p.strip() for p in set_clause.split(",")]:
            ...
            if val == "true":
                ...
                assignments[col] = str(params_[placeholder_idx])
                placeholder_idx += 1
            else:
                raise ValueError(f"Unsupported SET value in {sql_!r}: {part!r}")
        service_client_.table("transactions").update(assignments).eq("id", str(txn_id)).execute()

    _write(sql, params)
    update_call = service_client_.table.return_value.update.call_args
    eq_call = service_client_.table.return_value.update.return_value.eq.call_args
    return {
        "assignments": update_call[0][0],
        "eq_col": eq_call[0][0],
        "eq_val": eq_call[0][1],
    }


def test_update_adapter_self_transfer_with_link():
    """Shape 1: UPDATE transactions SET is_self_transfer = true, linked_txn_id = %s WHERE id = %s"""
    debit_id = uuid4()
    credit_id = uuid4()
    result = _invoke_write_adapter(
        "UPDATE transactions SET is_self_transfer = true, linked_txn_id = %s WHERE id = %s",
        (debit_id, credit_id),
    )
    assert result["assignments"] == {"is_self_transfer": True, "linked_txn_id": str(debit_id)}
    assert result["eq_col"] == "id"
    assert result["eq_val"] == str(credit_id)


def test_update_adapter_self_transfer_only():
    """Shape 2: UPDATE transactions SET is_self_transfer = true WHERE id = %s"""
    txn_id = uuid4()
    result = _invoke_write_adapter(
        "UPDATE transactions SET is_self_transfer = true WHERE id = %s",
        (txn_id,),
    )
    assert result["assignments"] == {"is_self_transfer": True}
    assert result["eq_val"] == str(txn_id)


def test_update_adapter_refund_with_link():
    """Shape 3: UPDATE transactions SET is_refund = true, linked_txn_id = %s WHERE id = %s"""
    original_id = uuid4()
    credit_id = uuid4()
    result = _invoke_write_adapter(
    ...
    assert result["assignments"] == {"is_refund": True, "linked_txn_id": str(original_id)}
    assert result["eq_val"] == str(credit_id)


def test_update_adapter_no_match_processed():
    """Shape 4: UPDATE transactions SET is_refund = false, is_self_transfer = false WHERE id = %s"""
    txn_id = uuid4()
    result = _invoke_write_adapter(
        "UPDATE transactions SET is_refund = false, is_self_transfer = false WHERE id = %s",
        (txn_id,),
    )
    assert result["assignments"] == {"is_refund": False, "is_self_transfer": False}


def test_update_adapter_unknown_shape_raises():
    """Unknown UPDATE shape must fail loud (not silently no-op)."""
    with pytest.raises(ValueError, match="Unsupported UPDATE shape"):
        _invoke_write_adapter("DELETE FROM transactions WHERE id = %s", (uuid4(),))


def test_update_adapter_unknown_set_value_raises():
    """Unknown SET value (e.g. a literal int) must fail loud."""
    with pytest.raises(ValueError, match="Unsupported SET value"):
        _invoke_write_adapter(
            "UPDATE transactions SET is_refund = 1 WHERE id = %s",
            (uuid4(),),
        )
